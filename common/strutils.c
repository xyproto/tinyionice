...
int streq_paths(const char* a, const char* b)
{
    while (a && b) {
        ...
        */
        ...
        if (a_sz + b_sz == 1 && ((a_seg && *a_seg == '/') || (b_seg && *b_seg == '/')))
            return 1;
...
static int test_strdup_to_member(int argc, char* argv[])
{
    ...
    if (strcmp(xx->name, argv[1]) != 0 && strcmp(xx->value, argv[2]) != 0)
...
static int test_strutils_sizes(int argc, char* argv[])
{
    ...
    hum2 = size_to_human_string(SIZE_SUFFIX_3LETTER | SIZE_SUFFIX_SPACE, size);
    hum3 = size_to_human_string(SIZE_SUFFIX_3LETTER | SIZE_SUFFIX_SPACE | SIZE_DECIMAL_2DIGITS, size);
